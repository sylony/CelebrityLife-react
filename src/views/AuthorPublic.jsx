...
import { FaCircleUser, FaRegThumbsUp, FaRegThumbsDown, FaRegPenToSquare, FaClipboardList, FaAlignJustify, FaUsers, FaArrowLeft, FaSearchengin, FaXmark, FaHeart } from "react-icons/fa6";
...
function AuthorPublic() {
    ...
    const id = queryParams.get('author_id'); // 获取"id"查询参数
    ...
    const getShare = () => {
        ...
        $AXIOS.get(`/api/share/get?author_id=${id}`).then(res => {
            console.log(res);
        ...
        <>
            <header className='flex items-center px-2 md:px-4 h-16 shadow-md justify-center md:justify-between'>
                <div className='flex'>
                    ...
                    <h2 onClick={_ => { navigate(`/people?id=${userInfo.id}`) }}
                        className="text-xl mb-0">
                        Author public share: {userInfo.username}
                    </h2>
                </div>
            ...
            <div className='flex justify-center mt-5' >
                <p onClick={_ => { navigate(`/people?id=${userInfo.id}`) }}
                    className='text-2xl font-bold hover:text-rose-400 cursor-pointer'>{userInfo.username}</p>
            </div>
